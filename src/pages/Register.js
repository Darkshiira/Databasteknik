      ...
      if (res.status === 201) {
        ...
        switch (res.status) {
          case 409:
            ...
            setFailedreg(true);
            setMsg(
              "Username is already being used by someone else, please choose another!"
            );
            break;
          case 418:
            setSuccecclog(false);
            setFailedreg(true);
            setMsg(
              "Oops, make sure your username and password is long enough."
            );
            break;
          default:
  ...
  return (
    ...
    <div>
      {successlog === false ? ( //Form is shown until successful registration have been made
        ...
        <div>
          {" "}
          {/*After successful registration, user is directed to log in*/}
          <Msgbox text1={msg} />
      ...
      {failedreg === true ? (
        <div>
          <Msgbox text1={msg} />{" "}
          {/*After failed registration, user is informed of why it failed*/}
        </div>
      ) : null}
    </div>
  );
